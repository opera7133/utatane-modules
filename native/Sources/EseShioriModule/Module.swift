...
private func load(_ input: UnsafeMutableRawPointer?, _ length: Int32, utf8Only: Bool) -> Int32 {
    ...
    let stateURL: URL?
    if let statePath = ProcessInfo.processInfo.environment["ESE_SHIORI_STATE_PATH"] {
        guard statePath.hasPrefix("/"), !statePath.contains("\0") else { return 0 }
        stateURL = URL(fileURLWithPath: statePath)
    } else {
        stateURL = nil
    }
    session.engine = try? EseShioriSession(
...
@_cdecl("loadu")
public func eseLoadUTF8(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: true)
}

@_cdecl("load")
public func eseLoad(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: false)
}

@_cdecl("unload")
public func eseUnload() -> Int32 {
    let session = Session.shared
    guard session.lock.try() else { return 0 }
    defer { session.lock.unlock() }
...
@_cdecl("request")
public func eseRequest(_ input: UnsafeMutableRawPointer?,
                       _ length: UnsafeMutablePointer<Int32>?) -> UnsafeMutableRawPointer?
{
    ...
    case "euc-kr", "euckr": charset = "EUC-KR"
    default: return nil
    }
