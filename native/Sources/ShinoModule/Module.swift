...
private func load(_ input: UnsafeMutableRawPointer?, _ length: Int32, utf8Only: Bool) -> Int32 {
    ...
    let stateURL: URL?
    if let statePath = ProcessInfo.processInfo.environment["SHINO_STATE_PATH"] {
        guard statePath.hasPrefix("/"), !statePath.contains("\0") else { return 0 }
        stateURL = URL(fileURLWithPath: statePath)
    } else {
        stateURL = nil
    }
    session.engine = try? ShinoSession(
...
@_cdecl("loadu")
public func shinoLoadUTF8(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: true)
}

@_cdecl("load")
public func shinoLoad(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: false)
}

@_cdecl("unload")
public func shinoUnload() -> Int32 {
    let session = Session.shared
    guard session.lock.try() else { return 0 }
    defer { session.lock.unlock() }
...
@_cdecl("request")
public func shinoRequest(_ input: UnsafeMutableRawPointer?,
