...
import NiseShioriCore

private final class Session: @unchecked Sendable {
    static let shared = Session()
    let lock = NSLock()
    var engine: NiseShioriSession?
}
...
@_cdecl("loadu")
public func niseLoadUTF8(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: true)
}

@_cdecl("load")
public func niseLoad(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: false)
}

@_cdecl("unload")
public func niseUnload() -> Int32 {
    let session = Session.shared
    guard session.lock.try() else { return 0 }
    defer { session.lock.unlock() }
...
@_cdecl("request")
public func niseRequest(_ input: UnsafeMutableRawPointer?,
