...
private func load(_ input: UnsafeMutableRawPointer?, _ length: Int32, utf8Only: Bool) -> Int32 {
    ...
    session.engine = try? KawariSession(masterDirectoryURL: URL(fileURLWithPath: path))
    return session.engine == nil ? 0 : 1
}

@_cdecl("loadu")
public func kawariLoadUTF8(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: true)
}

@_cdecl("load")
public func kawariLoad(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: false)
}

@_cdecl("unload")
public func kawariUnload() -> Int32 {
    let session = Session.shared
    guard session.lock.try() else { return 0 }
    defer { session.lock.unlock() }
    guard session.engine != nil else { return 1 }
    session.engine = nil
    return 1
}

@_cdecl("request")
public func kawariRequest(_ input: UnsafeMutableRawPointer?,
                        _ length: UnsafeMutablePointer<Int32>?) -> UnsafeMutableRawPointer?
{
    ...
    guard let engine = session.engine,
          var response = try? engine.request(request),
          let encoded = LegacyTextDecoder.encode({
              response.headers = ShioriHeaders(response.headers.entries.filter {
                  $0.name.caseInsensitiveCompare("Charset") != .orderedSame
              })
              response.headers.append(name: "Charset", value: charset)
