...
private func load(_ input: UnsafeMutableRawPointer?, _ length: Int32, utf8Only: Bool) -> Int32 {
    ...
    let stateURL: URL?
    if let statePath = ProcessInfo.processInfo.environment["YUHNA_STATE_PATH"] {
        guard statePath.hasPrefix("/"), !statePath.contains("\0") else { return 0 }
        stateURL = URL(fileURLWithPath: statePath)
    } else {
        stateURL = nil
    }
    session.engine = try? YuhnaSession(
...
@_cdecl("loadu")
public func yuhnaLoadUTF8(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: true)
}

@_cdecl("load")
public func yuhnaLoad(_ input: UnsafeMutableRawPointer?, _ length: Int32) -> Int32 {
    load(input, length, utf8Only: false)
}

@_cdecl("unload")
public func yuhnaUnload() -> Int32 {
    let session = Session.shared
    guard session.lock.try() else { return 0 }
    defer { session.lock.unlock() }
...
@_cdecl("request")
public func yuhnaRequest(_ input: UnsafeMutableRawPointer?,
