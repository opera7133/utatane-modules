...
int main(int argc, char **argv) {
    ...
    const char *saori = "GET SHIORI/3.0\r\nCharset: UTF-8\r\nID: OnSaori\r\n\r\n";
    ...
    result = send_request(request, saori, strlen(saori), &n);
    assert(result && n > 0);
    text = calloc((size_t)n + 1, 1);
    assert(text); memcpy(text, result, (size_t)n); free(result);
    assert(strstr(text, "Value: value|extra\r\n")); free(text);
    assert(unload() == 1);
    ...
    puts("PASS shino: conventional ABI, UTF-8/Shift_JIS, persisted state");
}
