...
状態は既定で`ghost/master/yuhna-state.json`へ保存します。ベースウェアから`YUHNA_STATE_PATH`に絶対パスを渡すと保存先を変更できます。このdylibは結奈SecondEdition Unit 10の独自再実装で、編集機能などすべての機能は再現していません。再配布時はZIPの`LICENSES/`も確認してください。
