...
macOS 14以降向けのSHIORIです。ZIPの`satori/lib/libsatori.dylib`を`ghost/master/`へ置き、`descript.txt`に追記します。
...
通常の`loadu` / `load`・`request`・`unload`で呼び出せます。パス・電文はUTF-8、長さは32ビット符号付き整数。入力はライブラリが、応答は呼び出し側が`free`します。1プロセスにつき1セッションです。里々内部の文字コードはShift_JISなので、そこで表現できない文字を含む要求は処理できません。
...
ライセンスはBSD 2-Clause。接続部分はMITです。出典と固定コミットはZIP内の`module.json`に記録しています。
