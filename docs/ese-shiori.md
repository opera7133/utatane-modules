...
状態は既定で`ghost/master/ese-shiori-state.json`へ保存します。ベースウェアから`UTATANE_GHOST_STATE_DIR`に絶対パスを渡すと、そのフォルダに保存します。`$WRITEFILE`の出力は保存先と同じ親フォルダの`ese-shiori-files/`に置きます。
