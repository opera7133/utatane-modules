...
`ai*.txt`と`ai*.dtx`を読み込み、状態を`ghost/master/nise-shiori-state.json`に保存します。ベースウェアから`UTATANE_GHOST_STATE_DIR`に絶対パスを渡すと、そのフォルダに保存します。保存先には書き込み権限が必要です。元のWindows版が持つすべての機能を再現するものではありません。
