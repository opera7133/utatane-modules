...
状態は既定で`ghost/master/shino-state.json`へ保存します。ベースウェアから`SHINO_STATE_PATH`に絶対パスを渡すと保存先を変更できます。`%saori`はゴースト直下の`filename.dylib`または`libfilename.dylib`を読み込みます。共通導入版SAORIを使う場合は`SHINO_SAORI_ROOT`に、`<名前>/lib/lib<名前>.dylib`を含むフォルダの絶対パスを指定してください。
